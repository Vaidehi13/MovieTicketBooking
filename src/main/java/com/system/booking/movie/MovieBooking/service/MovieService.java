...
public class MovieService {
    ...
    public void addNewMovie(Movie movie) {
        ...
        for(Screen s : screens){
            Screen screen = screenRepository.findById(s.getScreen_id())
                    .orElseThrow(()-> new RuntimeException("There is no screen with this id"));
    ...
    public void updateMovie(Movie movie) {
        Movie movieInDb = movieRepository.findById(movie.getMovie_id()).orElse(null);
        if (movieInDb != null) {
            ...
            for (Screen s : updateScreens) {
                ...
                for (Screen sn : screensInDb) {
                    if (sn.getScreen_id() == s.getScreen_id()) {
                        existsInDb = true;
                        Screen screenInDb = screenRepository.findById(sn.getScreen_id()).orElse(null);
                        if (screenInDb != null) {
                            Theatre theatre = theatreRepository.findById(screenInDb.getTheatre().getTheatre_id()).orElse(null);
                            if (theatre != null) {
